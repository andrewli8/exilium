...
export function buildMcpServer(service: ExiliumService, defaultGame: Game = 'poe1'): McpServer {
  ...
  server.registerTool(
    ...
    async ({ game, league, horizon_hours }) => {
      ...
      return json({
        ...report,
        methodology: 'Signal onsets only (a signal active at the previous tick is the same episode — including across data gaps, so a long-running signal spanning a gap still counts once); wall-clock horizons; baselineHitRate is the fraction of ALL items that moved in the predicted direction over the same windows — a detector only has edge above that number.',
      });
    },
  );
