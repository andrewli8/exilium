...
2. Integration tests for the CLI command layer.
   Evidence: src/cli.ts is excluded from coverage (vitest.config.ts) and is now ~500 lines of wiring where real bugs have lived (the dashboard filter bug fixed in commit "fix: dashboard opportunities respect min-edge"); no test executes the actual commands.
   Acceptance: a new tests/cli-integration.test.ts spawns the real CLI (price, snapshot, opps, journal add/list, watches add/list/rm) against a temp EXILIUM_DB fixture and asserts on stdout; failing before, passing after; npm test green.

3. Recorded real-payload fixtures for the normalizer.
...
4. Backtest harness: replay stored history and measure detector hit rate.
