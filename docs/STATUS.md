...
1. Trade journal: record and review trade-plan outcomes.
   Evidence: src/signals/trade-plan.ts:24; the final plan step tells users to "Record the outcome (filled / partial / no-fill)" but nothing in the codebase stores outcomes, and examples/03-common-workflows.md asks users to report outcomes with nowhere for them to land.
   Acceptance: a new `record_outcome` MCP tool and `exilium journal` CLI command persist outcomes (opportunity id, action, outcome, note) in SQLite and `exilium journal` prints them with a summary; new vitest file tests/journal.test.ts fails before implementation and passes after; npm test green.

2. CLI management for watches (list, add, remove, events).
...
3. TUI pane for fired watch events.
...
4. Dashboard opportunity list should respect min-edge and exclude experimental by default.
