...
For every listing, Exilium converts the asking price to chaos and checks local poe.ninja data. The board is one flat table of live listings — columns `PRICE`, `PROFIT`, `TIME`, `REWARD` — sorted newest first so fresh opportunities surface at the top. Rows at or above the profit threshold render bold; below-threshold and unknown-price rows stay visible but dimmed, so you can still judge (and travel to) them yourself. The threshold line above the table shows the active floor. The default floor is **+20%**; a per-search floor set in the configure overlay (`c`) wins over it, and the session floor can be changed with `--min-margin N`, `snipe.minMarginPct`, or `f` during a run. When no listings have arrived yet, the board lists each watched search and its connection state instead.
