...
On a qualifying new live hit you get a new bold table row, desktop notification, optional sound, and a `queued` entry in `~/.exilium/snipes.jsonl`. Nothing opens, travels, sends, or copies a whisper merely because the listing arrived. Press Enter on the selected row to travel to that listing's hideout — a green `Travel sent` confirms the click — or Shift+Enter to inspect every listing from that row's search; Shift+Tab or Escape returns to the board. If a listing is sold, Exilium removes it without automatically traveling to its replacement. Background socket, seed, and rate-limit diagnostics are written to `~/.exilium/snipe-runtime.log` so they do not corrupt the interactive screen.
